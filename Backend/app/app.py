    ...
    try:
        ...
        if result is not None:
            ...
            if result["isAuto"]:
                ...
                print(result["target"],result["projectFolderPath"],result["projectType"])
                manual_mode=training()
                plotFilePath=manual_mode.model_plot(pickleFilePath,cleanDataPath,result["target"],result["projectFolderPath"],result["projectType"])
                print(plotFilePath)
                try:
                    Project21Database.update_one(settings.DB_COLLECTION_PROJECT,{"projectID":projectID},{
                        "$set": {
...
def get_hyper_parameters(preprocessJSONFormData:dict, userID:int, projectID:int):
    ...
    if os.path.exists(cleanDataPath):
        ...
        json_data={"modelyaml":yaml_json,"dataID":dataID}
        return json_data
...
def start_manual_training(userID:int,projectID:int,dataID:int,configModelJSONData:Optional[List]):
    ...
    result_data=Project21Database.find_one(settings.DB_COLLECTION_DATA,{"dataID":dataID,"belongsToUserID":userID,"belongsToProjectID":projectID})
    if result_data is not None:
        cleanDataPath=result_data["cleanDataPath"]
